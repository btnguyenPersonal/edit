	...
	if (state->mode == SHORTCUTS && state->filename == "") {
		...
		if (state->recording == false && state->mode == SHORTCUTS) {
			std::vector<diffLine> diff =
				generateDiff(state->previousState, state->data);
			if (diff.size() != 0) {
				// TODO helper function for recording jumplist
				if (state->mode == SHORTCUTS &&
				    c != ctrl('z') && c != ctrl('q')) {
					if (state->jumplist.list.size() > 0) {
						auto pos = state->jumplist.list
								   .back();
						if (pos.row != state->row ||
						    pos.col != state->col) {
							state->jumplist.list.erase(
								state->jumplist.list
										.begin() +
									state->jumplist
										.index +
		...
		for (uint32_t i = 0; i < 10; i++) {
			if (state->harpoonFiles.count(i) > 0 &&
			    state->harpoonFiles[i] == state->filename) {
