...
std::string trimLeadingComment(State *state, std::string line)
{
	...
	for (size_t i = 0; i < line.length(); i++) {
		if (line[i] != getIndentCharacter(state)) {
			if (line.substr(i, state->commentSymbol.length()) ==
			    state->commentSymbol) {
				if (line.substr(i,
						state->commentSymbol.length() +
							1) ==
				    state->commentSymbol + " ") {
					outputLine =
						line.substr(0, i) +
...
void toggleCommentHelper(State *state, uint32_t row, int32_t commentIndex)
{
	...
	if (commentIndex == -1) {
		...
		if (isCommentWithSpace(state, line)) {
			state->data[row] =
				line.substr(0, i) +
				line.substr(i + state->commentSymbol.length() +
					    1);
			return;
	...
	if (line.length() != 0) {
		int32_t spaces =
			commentIndex != -1 ?
				commentIndex :
				getNumLeadingIndentCharacters(state, line);
		state->data[row] = line.substr(0, spaces) +
				   state->commentSymbol + ' ' +
				   line.substr(spaces);
	}
}
...
{
	ltrim(line);
	return line.substr(0, state->commentSymbol.length() + 1) ==
	       state->commentSymbol + ' ';
}
...
{
	ltrim(line);
	return line.substr(0, state->commentSymbol.length()) ==
	       state->commentSymbol;
}
...
void toggleCommentLines(State *state, Bounds bounds)
{
	...
	if (foundNonComment) {
		...
		for (size_t i = bounds.minR; i <= bounds.maxR; i++) {
			int32_t indent = getNumLeadingIndentCharacters(
				state, state->data[i]);
