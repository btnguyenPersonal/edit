...
uint32_t applyDiff(State *state, std::vector<diffLine> diff, bool reverse)
{
	std::sort(diff.begin(), diff.end(),
		  [reverse](const diffLine &a, const diffLine &b) {
			  if (b.add == a.add) {
	...
	for (const auto &dl : diff) {
		if ((!reverse && dl.add) || (reverse && !dl.add)) {
			...
			if ((int32_t)dl.lineNum + offsetNeg <
			    static_cast<int32_t>(state->data.size())) {
				state->data.erase(state->data.begin() +
						  dl.lineNum + offsetNeg);
			}
			offsetNeg--;
		}
	}
...
std::vector<diffLine> backtrack(const std::vector<std::vector<int32_t> > &trace,
				...
				const std::vector<std::string> &b, int32_t max)
{
	...
	for (int32_t d = static_cast<int32_t>(trace.size()) - 1, k, prev_k,
		     prev_x, prev_y;
	     x > 0 || y > 0; --d) {
		k = x - y;
		int32_t vk = offset + k;
		...
		if (x > prev_x && x != 0) {
			diff.insert(diff.begin(),
				    diffLine{ static_cast<uint32_t>(x - 1),
					      false, a[x - 1] });
		} else if (y > prev_y && y != 0) {
			diff.insert(diff.begin(),
				    diffLine{ static_cast<uint32_t>(y - 1),
					      true, b[y - 1] });
		}
...
std::vector<diffLine> generateDiff(const std::vector<std::string> &a,
...
std::vector<diffLine> generateFastDiff(const std::vector<std::string> &a,
				       const std::vector<std::string> &b)
{
