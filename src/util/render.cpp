...
void mvprintw_color(int32_t r, int32_t c, const char *formatString,
		    const char *cstring1, int32_t num, const char *cstring2,
		    int32_t color)
{
...
void mvprintw_color(int32_t r, int32_t c, const char *formatString, int32_t num,
		    int32_t color)
{
	attron(COLOR_PAIR(color));
...
void mvprintw_color(int32_t r, int32_t c, const char *formatString,
		    const char *cstring, int32_t color)
{
	attron(COLOR_PAIR(color));
...
void renderFileStack(State *state)
{
	...
	if ((int32_t)state->fileStackIndex < start - (int32_t)state->maxY / 2) {
		start = (int32_t)state->fileStackIndex +
	...
	for (int32_t i = start; i >= 0; i--) {
		...
		if (state->maxX / 2 > filename.length()) {
			filename += std::string(
				state->maxX / 2 - filename.length() - 1, ' ');
		}
		mvprintw_color(renderIndex, state->maxX / 2, "|%s",
			       filename.c_str(),
			       i == (int32_t)state->fileStackIndex ?
				       invertColor(YELLOW) :
				       GREY);
		renderIndex++;
	}
	for (int32_t i = renderIndex; i <= (int32_t)state->maxY; i++) {
		mvprintw_color(i, state->maxX / 2, "|%s",
			       std::string(state->maxX / 2, ' ').c_str(), GREY);
		renderIndex++;
	}
}
...
int32_t renderStatusBar(State *state)
{
	...
	if (state->mode == NAMING) {
		mvprintw_color(0, offset, "name: %s", state->name.query.c_str(),
			       WHITE);
		offset += state->name.cursor + 6;
		return offset;
	} else if (state->mode == COMMANDLINE) {
		mvprintw_color(0, offset, ":%s",
			       state->commandLine.query.c_str(), WHITE);
		...
		if (state->grepPath != "") {
			...
			mvprintw_color(0, offset, "%s", gPath.c_str(),
				       state->showAllGrep ? DARKGREEN : GREEN);
		}
		offset += gPath.length();
		mvprintw_color(0, offset, "> %s", state->grep.query.c_str(),
			       state->showAllGrep ? DARKGREEN : GREEN);
		renderNumMatches(offset + state->grep.query.length() + 4,
		...
		mvprintw_color(0, offset + 2, "%s",
			       state->findFile.query.c_str(),
			       state->selectAll ? invertColor(YELLOW) : YELLOW);
		renderNumMatches(offset + state->findFile.query.length() + 4,
		...
		mvprintw_color(0, offset, "/%s", displayQuery.c_str(),
			       state->searchFail ? RED : GREEN);
		if (state->replacing) {
			offset += displayQuery.length() + 1;
			mvprintw_color(0, offset, "/%s",
				       state->replace.query.c_str(), MAGENTA);
	...
	auto displayFileName =
		setStringToLength(state->filename, state->maxX - 30);
	mvprintw_color(0, state->maxX - (displayFileName.length() + 2),
		       "\"%s\"", displayFileName.c_str(),
		       state->unsavedFile ? GREY : WHITE);
	if (state->status.length() > 0) {
		mvprintw_color(0,
			       (state->maxX / 2) - (state->status.length() / 2),
			       "%s", state->status.c_str(), RED);
	}

	if (state->options.showmode) {
		std::string modename = getDisplayModeName(state);
		mvprintw_color(0, offset, " --%s-- ", modename.c_str(),
			       getModeColor(state));
...
int32_t getSearchColor(State *state, int32_t row, uint32_t startOfSearch,
		       std::string query, bool grep)
{
	if (state->row == (uint32_t)row &&
	    startOfSearch + query.length() >= state->col &&
	       ...
	       bool advance)
{
	if (' ' <= c && c <= '~') {
		...
		for (uint32_t i = 0; i < state->options.indent; i++) {
			mvaddch_color(row, col + getLineNumberOffset(state),
				      '.', color == WHITE ? GREY : color);
			advancePosition(state, row, col);
		}
		return;
	} else if (' ' <= unctrl(c) && unctrl(c) <= '~') {
		mvaddch_color(row, col + getLineNumberOffset(state), unctrl(c),
			      invertColor(MAGENTA));
	} else {
		mvaddch_color(row, col + getLineNumberOffset(state), ' ',
			      invertColor(MAGENTA));
...
{
	return ((int32_t)state->visual.row <= row &&
		row <= (int32_t)state->row) ||
	       ((int32_t)state->row <= row &&
		row <= (int32_t)state->visual.row);
}

uint32_t renderAutoComplete(State *state, uint32_t renderRow,
			    uint32_t renderCol)
{
	if (state->mode == TYPING || state->mode == MULTICURSOR) {
		...
		if (state->col + 1 >= state->data[state->row].length() ||
			...
			for (uint32_t i = 0; i < completion.length(); i++) {
				printChar(state, renderRow, renderCol + i,
					  completion[i], GREY);
			}
...
void renderGrepOutput(State *state)
{
	...
	for (uint32_t i = index;
	     i < state->grepOutput.size() && i < index + state->maxY; i++) {
		color = isTestFile(state->grepOutput[i].path.string()) ?
				ORANGE :
				WHITE;
		mvprintw_color(renderIndex, 0, "%s:%d %s\n",
			       ...
			       i == state->grep.selection ? invertColor(color) :
							    color);
		renderIndex++;
	}
}
...
{
	uint32_t index;
	if ((int32_t)state->findFile.selection - ((int32_t)state->maxY / 2) >
	    0) {
...
{
	if (state->mode == BLAME && state->blame.size() >= state->data.size() &&
	    i < (int32_t)state->data.size()) {
...
Cursor renderVisibleLines(State *state)
{
	...
	for (int32_t i = state->windowPosition.row;
	     i < (int32_t)state->data.size() &&
	     i < (int32_t)(state->maxY + state->windowPosition.row) - 1;
	     i++) {
		printLineNumber(state, i, currentRenderRow);
		currentRenderRow =
			printLineContent(state, i, currentRenderRow, &cursor);
	}
	return cursor;
}
...
void printLineNumber(State *state, int32_t row, int32_t renderRow)
{
	...
	bool isLogging = getLoggingRegex(state) != "" &&
			 std::regex_search(state->data[row],
					   std::regex(getLoggingRegex(state)));
	bool endsWithSpace = state->data[row].back() == ' ';
	bool isOnMark = (int32_t)state->mark.mark == row &&
			state->mark.filename == state->filename;
	int32_t color = BLACK;
	...
	if (state->mode == BLAME) {
		mvprintw_color(
			renderRow, border + 6, "%-65s",
			getRenderBlameString(state, row).substr(0, 65).c_str(),
			row == (int32_t)state->row ? invertColor(WHITE) :
						     WHITE);
	}
}
...
int32_t printLineContent(State *state, int32_t row, int32_t renderRow,
			 Cursor *cursor)
{
	if (isRowColInVisual(state, row, 0) == true &&
	    state->data[row].length() == 0) {
		...
		while (col < state->data[row].length()) {
			if (state->showGrep) {
				if (searchCounter == 0 &&
				    isInQuery(state, row, col,
					      state->grep.query)) {
					searchCounter =
						state->grep.query.length();
					startOfSearch = col;
				}
			} else {
				if (searchCounter == 0 &&
			...
			if (!isInString &&
			    state->data[row].substr(
				    col, state->commentSymbol.length()) ==
				    state->commentSymbol &&
			    state->commentSymbol != "") {
				isComment = true;
			}
			if (col >= state->windowPosition.col) {
				if (state->replacing && searchCounter != 0) {
					for (uint32_t i = 0;
					     i < state->replace.query.length();
					     i++) {
						printChar(
							state, renderRow,
							renderCol,
							state->replace.query[i],
							getSearchColor(
								state, row,
								startOfSearch,
								state->search
					...
					int32_t color;
					if (state->matching.row ==
						    (uint32_t)row &&
					    state->matching.col == col &&
					    (state->matching.row !=
						     state->row ||
					     state->matching.col !=
						     state->col)) {
					...
					printChar(state, renderRow, renderCol,
						  state->data[row][col], color,
						  true);
					col++;
					if (state->row == (uint32_t)row &&
					    state->col == col) {
...
void moveCursor(State *state, int32_t cursorOnStatusBar, Cursor editorCursor,
		Cursor fileExplorerCursor)
{
	if (cursorOnStatusBar != -1) {
		...
		move(editorCursor.row + 1,
		     editorCursor.col + getLineNumberOffset(state));
	}
}

int32_t renderFileExplorerNode(FileExplorerNode *node, int32_t r,
			       int32_t selectedRow, std::string startingSpaces,
			       bool isFileExplorerOpen,
			       int32_t fileExplorerWindowLine,
			       int32_t fileExplorerSize, Cursor &cursor)
{
	int32_t color;
	...
	if (row - fileExplorerWindowLine > 0) {
		...
		mvprintw_color(displayRow, offset, "%s", startingSpaces.c_str(),
			       WHITE);
		offset += startingSpaces.length();
		if (node->isFolder) {
			mvprintw_color(displayRow, offset, "%s",
				       node->isOpen ? "v" : ">", GREY);
			offset += 1;
		}
		offset += 1;
		...
		mvprintw_color(
			displayRow, offset,
			(std::string("%-") +
			 std::to_string(fileExplorerSize - offset - 1) +
			 std::string("s"))
				.c_str(),
			(int32_t)node->name.length() >=
					fileExplorerSize - offset - 1 ?
				(node->name.substr(0, fileExplorerSize - 3 -
							      offset - 1) +
				 "...")
					.c_str() :
				node->name
					...
					.c_str(),
			color);
		offset += node->name.substr(0, fileExplorerSize - offset - 1)
				  .length();
	}
	if (node->isOpen) {
		for (uint32_t i = 0; i < node->children.size(); i++) {
			row = renderFileExplorerNode(&node->children[i], row,
						     selectedRow,
						     startingSpaces + "  ",
						     isFileExplorerOpen,
						     fileExplorerWindowLine,
						     fileExplorerSize, cursor);
		}
	}
	return row;
}
...
{
	Cursor cursor{ -1, -1 };
	renderFileExplorerNode(state->fileExplorer, 0, state->fileExplorerIndex,
			       std::string(""), state->mode == FILEEXPLORER,
			       state->fileExplorerWindowLine,
			       state->fileExplorerSize, cursor);
	return cursor;
}
...
void initTerminal()
{
	...
	if (COLORS < 256) {
		endwin();
		std::cout << "Your terminal does not support 256 colors"
			  << std::endl;
		exit(1);
	}
	initColors();
