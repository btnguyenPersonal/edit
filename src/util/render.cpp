...
void insertPixels(State *state, std::vector<Pixel> *pixels, std::string s, int32_t color) {
	for (uint32_t i = 0; i < s.length(); i++) {
		insertPixel(state, pixels, s[i], color);
	}
}
...
int32_t renderStatusBar(State *state) {
	...
	if (state->numRepeats > 0) {
		rightSide = std::to_string(state->numRepeats) + rightSide;
	}
	...
	if (state->numRepeats > 0) {
		insertPixels(state, &pixels, std::to_string(state->numRepeats), RED);
	}
...
bool isMergeConflict(const std::string &str) {
	...
	if (str.length() == 7) {
		for (uint32_t i = 0; i < markers.size(); i++) {
			if (str[0] == markers[i][0]) {
				selectedMarker = i;
				break;
			}
		}
		if (selectedMarker != -1) {
			for (uint32_t i = 0; i < markers[selectedMarker].length(); i++) {
				if (str[i] != markers[selectedMarker][i]) {
					return false;
				}
			}
			return true;
